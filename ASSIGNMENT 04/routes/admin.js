...
router.post('/products/add', requireAdmin, async (req, res) => {
  try {
    ...
    const { name, price, category, badge, sizes, stock, description } = req.body;
    ...
    console.log(err);
    res.render('admin/add-product', { error: 'Something went wrong.', success: null });
  }
});
...
router.post('/products/edit/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const product = await Product.findById(req.params.id);
    res.render('admin/edit-product', { product, error: 'Something went wrong', success: null });
  }
});
...
// routes/admin.js  (add/edit product routes)

router.post('/products/add', async (req, res) => {
  try {
    const { name, price, category, badge, stock, description, imageUrls } = req.body;

    // Parse sizes (checkboxes send array or single string)
    const sizes = Array.isArray(req.body.sizes) ? req.body.sizes : [req.body.sizes].filter(Boolean);

    // Parse image URLs from Cloudinary (comma-separated)
    const images = imageUrls
      ? imageUrls.split(',').map(u => u.trim()).filter(Boolean)
      : [];

    const product = new Product({ name, price, category, badge, stock, description, sizes, images });
    await product.save();

    res.redirect('/admin/products?success=Product added');
  } catch (err) {
    res.render('admin/add-product', { error: err.message });
  }
});
...
router.post('/products/edit/:id', async (req, res) => {
  try {
    ...
    const update = { name, price, category, badge, stock, description, sizes };

    // Only replace images if new ones were uploaded
    if (imageUrls && imageUrls.trim()) {
      update.images = imageUrls.split(',').map(u => u.trim()).filter(Boolean);
    }
    ...
    const product = await Product.findById(req.params.id);
    res.render('admin/edit-product', { product, error: err.message, success: null });
  }
});
